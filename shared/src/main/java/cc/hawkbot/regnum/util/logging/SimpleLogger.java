 ...
 * be the <em>path</em> to a file, or the special values "System.out" and "System.err". Default is
 * "System.err".</li>
 *
 * <li><code>org.slf4j.simpleLogger.cacheOutputStream</code> - If the output
 * target is set to "System.out" or "System.err" (see preceding entry), by default, logs will be
 * output to the latest value referenced by
 * <code>System.out/err</code> variables. By setting this
 * parameter to true, the output stream will be cached, i.e. assigned once at initialization time
 * and re-used independently of the current value referenced by
 * <code>System.out/err</code>.
 * </li>
 *
 * <li><code>org.slf4j.simpleLogger.defaultLogLevel</code> - Default log level
 * for all instances of SimpleLogger. Must be one of ("trace", "debug", "info", "warn", "error" or
 * "off"). If not specified, defaults to "info".</li>
 *
 * <li><code>org.slf4j.simpleLogger.log.<em>a.b.c</em></code> - Logging detail
 * level for a SimpleLogger instance named "a.b.c". Right-side value must be one of "trace",
 * "debug", "info", "warn", "error" or "off". When a SimpleLogger named "a.b.c" is initialized, its
 * level is assigned from this property. If unspecified, the level of nearest parent logger will be
 ...
 * format to be used in the output messages. The pattern describing the date and time format is
 * defined by <a href= "http://docs.oracle.com/javase/1.5.0/docs/api/java/text/SimpleDateFormat.html">
 ...
 * also checks for a class loader resource named
...
class SimpleLogger extends MarkerIgnoringBase {
  ...
  public static final String CACHE_OUTPUT_STREAM_STRING_KEY =
      SimpleLogger.SYSTEM_PREFIX + "cacheOutputStream";
  ...
  public static final String SHOW_SHORT_LOG_NAME_KEY =
      SimpleLogger.SYSTEM_PREFIX + "showShortLogName";
  ...
  SimpleLogger(String name, Level level) {
    this.name = name;

    this.currentLogLevel = level.toInt();
  }
  ...
  public void log(LoggingEvent event) {
    ...
    FormattingTuple tp = MessageFormatter
        .arrayFormat(event.getMessage(), event.getArgumentArray(), event.getThrowable());
  ...
  private String getFormattedDate() {
    Date now = new Date();
    String dateText;
    dateText = new SimpleDateFormat().format(now);
    return dateText;
  }
